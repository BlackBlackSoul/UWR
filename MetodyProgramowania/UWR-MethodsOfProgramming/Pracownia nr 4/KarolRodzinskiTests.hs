...
tests =
  ...
  , Test "negToIntegerErr" (SrcString "1 + not 2") TypeError
  ...
  , Test "simpleInputIn"       (SrcString "input x, y in x * 1 - y div 4") (Eval [1,12] (Value 1))
  ...
  , Test "checkBoolInIf"      (SrcString "input x in if x div 2 >= 1 then x * 3 else x div 3") (Eval [24] (Value 8))
  , Test "undefinedVariables"      (SrcString "input x y in x*y + z") TypeError
  , Test "boolErrorInLet"      (SrcString "input x, y, z in let  x >= y in z") TypeError
  ...
  , Test "boolOperatorToInteger" (SrcString "1 + not 2") TypeError
