...
lista::wezel& lista::operator[](int pos) const {
    if (pos < 0 || pos >= this->size)
        throw std::string("Brak elementu o takim indeksie!\n");
...
void lista::wezel::operator delete(void *address) {
    try {
        ...
        std::string newmess = "operator delete(void*):" + message;
        throw std::string(newmess);
    }
}
...
void lista::wstaw(int pos, double val) {
    if (pos < 0)
        throw std::string("indeks mniejszy od 0!\n");
...
double lista::usun(int pos) {
    if (size <= 0)
        throw std::string("Struktora pusta, nie ma czego usuwac!\n");

    else if (pos >= this->size)
        throw std::string("Brak takiego indeksu!\n");
...
bool Memory::flags[Memory::size]; //1 - wolne, 0 - zajete

lista::wezel* Memory::getAddress() {
    int i;
    for (i = 0; i < size && flags[i] == 0; i++);
    if (i == size)
        throw std::string("Brak miejsca w strukturze!\n");
    else {
        flags[i] = 0;
        return &ground[i];
    }
...
void Memory::freeAddress(lista::wezel *address) {
    ...
    while (i != size && &ground[i] != address)
        i++;
    if (i == size)
        throw std::string("Miejsce pod wezel nie zostalo wziete z tej tablicy!\n");
    else
        flags[i] = 1;
}